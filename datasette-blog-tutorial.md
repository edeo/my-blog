...
Create a virtual environment so all your Python packages live in one place:
...
```bash
pip install datasette datasette-template-sql datasette-publish-fly sqlite-utils markdown
```

Create a `requirements.txt` so the GitHub Actions workflow uses the same versions:
...
**Deploy fails with "No such command 'fly'"**: Make sure `datasette-publish-fly` is in your `requirements.txt` and that the workflow includes the "Install flyctl" step.
